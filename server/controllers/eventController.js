...
export const createEvent = async (req, res) => {
  try {
    ...
    if (event.status === 'approved') {
      (async () => {
        try {
          const followers = await Follower.find({ club: clubId }).populate('student', 'email name');
          const emailList = followers.map(f => f.student?.email).filter(Boolean);
          
          if (emailList.length > 0) {
            const subject = `New Event Published: ${event.title}`;
            const html = `
              <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px;">
                <div style="background-color: #ec4899; color: white; padding: 20px; text-align: center;">
                  <h2 style="margin: 0;">🎉 New Event from ${club.name}!</h2>
                </div>
                <div style="padding: 20px;">
                  <p>Hello,</p>
                  <p>A club you follow, <strong>${club.name}</strong>, just published a new event: <strong>${event.title}</strong>.</p>
                  <div style="background-color: #f9fafb; padding: 15px; margin: 20px 0; border-left: 4px solid #ec4899;">
                    <p style="margin: 0 0 10px 0;"><strong>Date:</strong> ${new Date(event.date).toLocaleDateString()}</p>
                    <p style="margin: 0 0 10px 0;"><strong>Time:</strong> ${event.time}</p>
                    <p style="margin: 0;"><strong>Venue:</strong> ${event.venue}</p>
                  </div>
                  <p>Check out the details and register on your Campus Connect dashboard!</p>
                </div>
              </div>
            `;
            await Promise.all(emailList.map(email => sendEmail({ to: email, subject, html }).catch(err => console.error('Email failed:', err))));
          }
        } catch (err) {
...
export const approveEvent = async (req, res) => {
  try {
    ...
    (async () => {
      try {
        const club = await Club.findById(event.club);
        const followers = await Follower.find({ club: event.club }).populate('student', 'email name');
        const emailList = followers.map(f => f.student?.email).filter(Boolean);
        
        if (emailList.length > 0 && club) {
          const subject = `New Event Published: ${event.title}`;
          const html = `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px;">
              <div style="background-color: #ec4899; color: white; padding: 20px; text-align: center;">
