...
const ClubCard = ({ club, onManageProfile, onManageMedia, onViewFollowers }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition-shadow flex flex-col h-full group text-center cursor-pointer hover:-translate-y-1">
      ...
      {user?.role === 'ClubAdmin' && (user._id === club.createdBy || user._id === club.createdBy?._id) && (
        <div className="flex flex-col gap-2">
          ...
          <button 
            ...
            className="w-full py-2.5 rounded-xl text-sm font-bold bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-800 transition-colors"
          >
            View Followers
          </button>
        </div>
