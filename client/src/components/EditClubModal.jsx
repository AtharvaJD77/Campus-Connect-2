...
import { X, Building, Mail, Loader, Upload } from 'lucide-react';

const EditClubModal = ({ club, onClose, onClubUpdated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.put(`/clubs/${club._id}`, form);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-slate-700">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white">Manage Profile Information</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Logo URL or Upload</label>
          ...
          <div className="flex gap-3 pt-4 border-t border-gray-100 dark:border-slate-700">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 rounded-xl text-sm shadow transition-all disabled:opacity-50 flex items-center justify-center"
            >
              {loading ? <><Loader className="animate-spin h-4 w-4 mr-2" /> Saving...</> : 'Save Profile Section'}
            </button>
          </div>
        </form>
      </div>
